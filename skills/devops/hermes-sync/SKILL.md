...
- **Python Virtualenv Execution for Sync/Deploy:** Running `deploy_sync.py` or `sync_to_repo.py` directly with system `python3` fails with `ModuleNotFoundError: No module named 'yaml'` because `PyYAML` resides inside the Hermes virtual environment. Always execute sync/deploy scripts using `~/.hermes/hermes-agent/venv/bin/python3` or `sys.executable` within wrapper scripts.
