...
## Pitfalls
- **Malformed Remote URLs:** Ensure the Git remote URL in the repository template is a clean HTTPS URL (e.g., `https://github.com/user/repo.git`) and does *not* contain pre-baked authentication tokens. Hardcoded tokens in `.git/config` can cause "Bad hostname" errors during automated pushes. Always use dynamic token injection at runtime (e.g., `https://{token}@github.com/...`) to ensure secure and reliable authentication.
