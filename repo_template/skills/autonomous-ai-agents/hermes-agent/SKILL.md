...
### Handling API Rate Limits (429)
When API providers (like Gemini or OpenRouter) return a 429 Resource Exhausted error:
1. Verify status via `hermes auth list`.
2. Reset the credential state with `hermes auth reset <provider>`.
3. If the gateway process is active, the system will automatically retry using the reset credentials; you do NOT need to restart the gateway process, as doing so manually from a subprocess will be blocked.


## Model Throughput Optimization
To maximize API quota usage, rotate between high-throughput models rather than switching API keys manually.
1. Define a priority list of models in a rotation script.
2. Configure a cron job to switch the default model periodically:
   ```bash
   hermes cron create "every 5m" --script "rotate.sh"
   ```
   (Ensure `rotate.sh` is placed in `~/.hermes/scripts/`).
